...
class CameraGlRenderer(private val onCameraReady: (Surface) -> Unit) : android.opengl.GLSurfaceView.Renderer {
    ...
    private var width=1; private var height=1; private val texMatrix=FloatArray(16); var requestRender:(()->Unit)?=null
    ...
    override fun onSurfaceCreated(gl:GL10?,config:EGLConfig?){
        ...
        cameraTexture=SurfaceTexture(textureId).also{st->st.setDefaultBufferSize(1920,1080);st.setOnFrameAvailableListener({frameAvailable=true;requestRender?.invoke()},Handler(Looper.getMainLooper()))}
    ...
    override fun onDrawFrame(gl:GL10?){
        if(frameAvailable){cameraTexture?.updateTexImage();cameraTexture?.getTransformMatrix(texMatrix);timestamp=cameraTexture?.timestamp?:timestamp;frameAvailable=false}
        draw()
    ...
    companion object{private const val VS="attribute vec2 aPosition;attribute vec2 aTexCoord;uniform mat4 uTexMatrix;varying vec2 vTexCoord;void main(){gl_Position=vec4(aPosition,0.0,1.0);vTexCoord=(uTexMatrix*vec4(aTexCoord,0.0,1.0)).xy;}";private const val FS="#extension GL_OES_EGL_image_external : require\nprecision mediump float;uniform samplerExternalOES uTexture;uniform float uTint;varying vec2 vTexCoord;void main(){vec4 c=texture2D(uTexture,vTexCoord);float a=abs(uTint)*0.35;vec3 outc=uTint>=0.0?vec3(c.r*(1.0+a),c.g*(1.0-a),c.b*(1.0+a)):vec3(c.r*(1.0-a),c.g*(1.0+a),c.b*(1.0-a));gl_FragColor=vec4(clamp(outc,0.0,1.0),c.a);}"
    }
}
